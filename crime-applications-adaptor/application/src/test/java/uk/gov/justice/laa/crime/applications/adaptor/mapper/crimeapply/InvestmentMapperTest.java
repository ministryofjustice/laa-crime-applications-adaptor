...
class InvestmentMapperTest {
    ...
    @BeforeEach()
    void setUp() { investmentMapper = new InvestmentMapper(); }

    @ParameterizedTest
    @MethodSource("investmentTypeMappingTestData")
    void shouldMapFromCrimeApplyInvestmentTypeToCrimeApplicationsCapitalType(Investment.InvestmentType inputCrimeApplyInvestmentType,
                                                                          CapitalOther.CapitalType expectedCapitalType) {
        CapitalOther.CapitalType actualCapitalType = investmentMapper.mapInvestmentTypeToCapitalType(inputCrimeApplyInvestmentType);
    ...
    void shouldMapFromCrimeApplyInvestmentOwnershipTypeToCrimeApplicationsAccountOwner(Investment.OwnershipType inputCrimeApplyOwnershipType,
                                                                               CapitalOther.AccountOwner expectedAccountOwnerType) {
        CapitalOther.AccountOwner actualAccountOwnerType = investmentMapper.mapInvestmentOwnerToAccountOwner(inputCrimeApplyOwnershipType);
    ...
    private static Stream<Arguments> ownershipTypeMappingTestData() {
        return Stream.of(
                ...
                Arguments.of(Investment.OwnershipType.APPLICANT_AND_PARTNER, CapitalOther.AccountOwner.JOINT)
        );
    }
}
