...
public class OutgoingsMapper {
      ...
      for (Outgoing outgoing : outgoings) {
        ...
        if (outgoingsType.equals(HOUSING) && Objects.nonNull(housingPaymentType)) {
          assessmentDetail.setAssessmentDetailCode(
              String.valueOf(HousingDetails.findByValue((String) housingPaymentType).getCode()));
        } else {
  ...
  public String mapOtherHousingFeesNotes(List<Outgoing> outgoings, Object housingPaymentType) {
    ...
    for (Outgoing outgoing : outgoings) {
      ...
      if (HOUSING.equals(outgoingsType)
          && Objects.nonNull(housingPaymentType)
          && housingPaymentType.equals(BOARD_LODGINGS_TYPE)) {
